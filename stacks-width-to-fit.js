...
function processLayerTree(layer, isRoot, stats) {
  if (isRoot) {
    // The parent Stack gets Fit width
    setHorizontalSizing(layer, FlexSizing.Fit);
  } else {
    // Children also get Fit width
...
if (!doc) {
  ...
  if (selection.isEmpty) {
    ...
    if (!validSelectionFound) {
      ...
      sketch.UI.message(`✅ Success: Width set to Fit for parent and ${stats.childCount} nested layer(s).`);
    }
  }
}