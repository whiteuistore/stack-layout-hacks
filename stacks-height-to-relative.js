...
function processLayerTree(layer, isRoot, stats) {
  if (isRoot) {
    // The parent Stack gets Relative height
    setVerticalSizing(layer, FlexSizing.Relative);
  } else {
    // Children also get Relative height
...
if (!doc) {
  ...
  if (selection.isEmpty) {
    ...
    if (!validSelectionFound) {
      ...
      sketch.UI.message(`✅ Success: Height set to Relative for parent and ${stats.childCount} nested layer(s).`);
    }
  }
}