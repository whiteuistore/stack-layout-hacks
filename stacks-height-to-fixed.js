...
function processLayerTree(layer, isRoot, stats) {
  if (isRoot) {
    // The parent Stack gets Fixed height
    setVerticalSizing(layer, FlexSizing.Fixed);
  } else {
    // Children also get Fixed height
...
if (!doc) {
  ...
  if (selection.isEmpty) {
    ...
    if (!validSelectionFound) {
      ...
      sketch.UI.message(`✅ Success: Height set to Fixed for parent and ${stats.childCount} nested layer(s).`);
    }
  }
}